...
def build_filter(build: builds.Build) -> bool:
    ...
    # Platform failing SMP + MCS, reasons can be:
    # - global timer, see https://github.com/seL4/seL4/issues/513
    # - IMX8MM_EVK is failing multicore tests
    if plat.name in ['SABRE', 'IMX8MM_EVK'] and build.is_smp() and build.is_mcs():
        return False

    # SCHED_CONTEXT_0014 fails on TX1, TX2 and ODROID_C4.
    # see https://github.com/seL4/seL4/issues/928
    if plat.name in ['TX1', 'TX2', 'ODROID_C4'] and \
       build.is_mcs() and build.is_smp() and build.is_hyp() and build.is_clang():
        return False

    # CACHEFLUSH0001 fails on ODROID_XU4.
    # see https://github.com/seL4/sel4test/issues/80
    ...
    # see https://github.com/seL4/sel4test/issues/44
